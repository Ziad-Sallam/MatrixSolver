...
class NewtonRaphsonMethod:
    ...
    def find_root(self, x0):
     try: 
        ...
        for self.n in range(1, self.max_iter + 1):
            ...
            x_new = self.format_significant_figures(x_old - (f_value / f_prime_value))
            
            if self.show_steps:
                print(f"Iteration {self.n}:\nX{self.n-1} = {self.format_significant_figures(x_old)}")
                self.ans += f"Iteration {self.n}:\nX{self.n-1} = {self.format_significant_figures(x_old)}\n"
                print(f"f(X{self.n-1}) = {self.format_significant_figures(f_value)}, f'(X{self.n-1}) = {self.format_significant_figures(f_prime_value)}")
                self.ans += f"f(X{self.n-1}) = {self.format_significant_figures(f_value)}, f'(X{self.n-1}) = {self.format_significant_figures(f_prime_value)}\n"
                print(f"X{self.n} = {self.format_significant_figures(x_old)} - ({self.format_significant_figures(f_value)} / {self.format_significant_figures(f_prime_value)}) = {self.format_significant_figures(x_new)}")
                self.ans += f"X{self.n} = {self.format_significant_figures(x_old)} - ({self.format_significant_figures(f_value)} / {self.format_significant_figures(f_prime_value)}) = {self.format_significant_figures(x_new)}\n"
    ...
    def solve(self):
        ...
        if root is not None:
            ...
            self.ans += f"Approximate relative error = {self.relative_error}%\n"
...
if __name__ == "__main__":
    ...
    solver = NewtonRaphsonMethod(equation, 1, 5, 0.0001, 10, True, 6)
    solver.solve()